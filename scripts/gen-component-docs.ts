...
const DESCRIPTIONS: Record<Lang, Record<string, Record<string, string>>> = {
  en: {
    ...
    navbar:      { brand: "Brand/logo text (left side)", items: "Menu items — a string, or { label, icon, active, open, children }", actions: "Right-aligned slots — { icon }, { avatar }, or { label }" },
    sidebar:     { items: "Menu entries — a string, a { section } header, or { label, icon, active, children }", active: "Active entry — label or zero-based index", collapsed: "Render as an icon-only rail" },
    ...
    button:      { label: "Button text", variant: "Visual style", icon: "Icon label (text only for now)" },
    ...
    tag:         { label: "Tag text" },
    marker:      { num: "Badge label — a number (or short string) cross-referenced to an external list", text: "Description revealed on hover", variant: "Color style" },
  ...
  ko: {
    ...
    navbar:      { brand: "브랜드/로고 텍스트 (왼쪽)", items: "메뉴 항목 — 문자열 또는 { label, icon, active, open, children }", actions: "오른쪽 정렬 슬롯 — { icon }, { avatar }, { label }" },
    sidebar:     { items: "메뉴 항목 — 문자열, { section } 헤더, 또는 { label, icon, active, children }", active: "활성 항목 — 라벨 또는 0-based 인덱스", collapsed: "아이콘만 있는 레일로 렌더" },
    ...
    button:      { label: "버튼 텍스트", variant: "시각 스타일", icon: "아이콘 라벨 (현재는 텍스트만)" },
    ...
    tag:         { label: "태그 텍스트" },
    marker:      { num: "뱃지 라벨 — 외부 설명 리스트와 매칭되는 번호(또는 짧은 문자열)", text: "호버 시 나타나는 설명", variant: "색상 스타일" },
    kbd:         { keys: "단축키의 키들" },
